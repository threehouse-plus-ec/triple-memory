...
- [x] **4. Letter-Group Dataset (Engine §8.3):** `packs/music/letter_groups.json` is present; parses as valid JSON; contains 20 curated triples covering 12 letters (B, C, D, F, H, L, M, P, R, S, T, V). Each group contains exactly 3 cards of 3 distinct card types. All labels within a group satisfy the `initial_letter` consistency check.
...
- **Non-Latin script pilot is narrow.** Cyrillic appears only via Tchaikovsky; Bengali and Devanagari only via Ravi Shankar. A cross-checker who reads either script natively should verify: (a) Чайковский, Щелкунчик (Russian); (b) রবি শংকর (Bengali); (c) राग जोग, सितार (Devanagari). At v0.1_draft, `local_display` for these entries is set to the conventional English form (*Tchaikovsky*, *Nutcracker*, *Ravi Shankar*, *Raga Jog*, *Sitar*) to avoid the overlay rendering the native-script string twice. The authoritative Latin-script local form (ISO-9 scientific transliteration versus conventional English versus a German-family transliteration) is deferred to cross-check — see Curation Statement §2.5.
...
4. **Decide the authoritative `local_display` romanisation.** v0.1_draft uses the conventional English form as `local_display` for native-script entries (Tchaikovsky, Ravi Shankar pilot) to avoid the overlay rendering the native-script string twice. The cross-check should decide whether that convention stays, or whether `local_display` should switch to an ISO-9 scientific transliteration (e.g. *Čajkovskij*) or to a German-family transliteration (*Tschaikowsky*) — the engine's overlay behaviour is defect-free under any of those three choices.
